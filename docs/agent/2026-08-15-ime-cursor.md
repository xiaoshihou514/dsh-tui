...
Kitty can retain a correct `get-text` cell model while its rendered right edge is visually missing after cursor movement, so model-level captures alone are insufficient verification. Cursor navigation now schedules a targeted repaint of the right-border cell after Ink has placed the hardware cursor. The repair saves the cursor, writes `│` at the frame's absolute final column, and restores the cursor; it therefore preserves IME placement and works for repeated no-op Ctrl+A presses as well as actual movement.
